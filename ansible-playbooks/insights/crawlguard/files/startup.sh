...
if [ -z ${CRAWLGUARD_APP_IMAGE+x} ]; then export CRAWLGUARD_APP_IMAGE="none"; fi

# Install dependencies
sudo dnf -y update && sudo dnf install -y aws-cli jq

AWS_REGION="eu-west-2"
PARAMETER_PATH="/application/crawlguard/redis"


# get docker image tag from parameter store
echo "retrieve versions"
exp_traefik_image=$(aws ssm get-parameter --name /application/crawlguard/redis/docker-images --query Parameter.Value --region $region --output text | jq -r '.["traefik"]')
exp_app_image=$(aws ssm get-parameter --name /application/crawlguard/redis/docker-images --query Parameter.Value --region $region --output text | jq -r '.["crawlguard-application"]')
...
# update app version
if [ "$CRAWLGUARD_APP_IMAGE" != "$exp_app_image" ] || [ "$set_app_image" != "$exp_app_image" ]; then
  sudo yq -i ".services.blue-web.image = \"$exp_app_image\"" /var/docker/compose.yml
  export CRAWLGUARD_APP_IMAGE="$exp_app_image"
  sudo sed -i "s|export CRAWLGUARD_APP_IMAGE=.*|export CRAWLGUARD_APP_IMAGE=\"$exp_app_image\"|g" /etc/environment
fi
