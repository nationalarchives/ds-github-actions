...
if [ -z ${REDIS_APP_IMAGE+x} ]; then export REDIS_APP_IMAGE="none"; fi


# get docker image tag from parameter store
echo "retrieve versions"
exp_traefik_image=$(aws ssm get-parameter --name /application/platform/redis/docker_images --query Parameter.Value --region $region --output text | jq -r '.["traefik"]')
exp_app_image=$(aws ssm get-parameter --name /application/platform/redis/docker_images --query Parameter.Value --region $region --output text | jq -r '.["redis-application"]')
...
# update app version
if [ "$REDIS_APP_IMAGE" != "$exp_app_image" ] || [ "$set_app_image" != "$exp_app_image" ]; then
  sudo yq -i ".services.blue-web.image = \"$exp_app_image\"" /var/docker/compose.yml
  export REDIS_APP_IMAGE="$exp_app_image"
  sudo sed -i "s|export REDIS_APP_IMAGE=.*|export REDIS_APP_IMAGE=\"$exp_app_image\"|g" /etc/environment
fi
