...
exp_app_image=$(aws ssm get-parameter --name /application/docker-images --query Parameter.Value --region $region --output text | jq -r '.["frontend-application"]')

# update traefik version if needed
if [ "$TRAEFIK_IMAGE" != "$exp_traefik_image" ]; then
  yq '.services.traefik.image = "$exp_traefik_image"' /var/docker/compose.traefik.yml
...
# update app version
if [ "$FRONTEND_APP_IMAGE" = "$exp_app_image" ]; then
  yq '.services.blue-web.image = "$exp_app_image"' /var/docker/compose.yml
